...
def addFacilityID(i):
	...
	if len(subList) != 0:
		...
		subList = subList + "_"
	return facilityList[i] + [str(i), str(subNum), subList]
...
keys = map(lambda fac: fac[3] + " - " + fac[3], facilityListID)
...
def substanceListfunc(row):
	...
	#NPRIID = ((str(int(NPRIID) + 10000000000)[1:]) if (len(NPRIID) > 0) else "")
	Sector = str(row[FieldIndexDict["NAICS"]])  + " - " + NAICSDictionary[str(int(NAICS))]
